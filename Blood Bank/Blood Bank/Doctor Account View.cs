...
{
    public partial class Doctor_Account_View : Form
    {
        public Doctor_Account_View()
        {
            InitializeComponent();
        }
        ...
        {
            string queary = "select firstName from employee where UserID ='" + txtUserID.Text + "' and userPassword = '" + txtPassword.Text + "' and empMode = 'Doctor'";
            string testName;
        ...
        {
            Application.Run(new Doctor_Account_Logged());
        }

        private void Doctor_Account_View_Load(object sender, EventArgs e)
        {
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (testName.Length > 0)
                {
                    ...
                    Thread edit = new Thread(openEditModeDoctor);
                    edit.SetApartmentState(ApartmentState.STA);
                    edit.Start();
                    this.Close();
                }
        ...
        private void openEditModeDoctor()
        {
            Application.Run(new Doctor_Account_Logged());
        }
        ...
        {
            Thread td = new Thread(goBackDoctorForm);
            td.SetApartmentState(ApartmentState.STA);
            td.Start();
            this.Close();
        }

        private void goBackDoctorForm()
        {
